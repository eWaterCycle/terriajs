...
const ViewingControls = createReactClass({
    ...
    getInitialState() {
        return {
            controlWidths: [null, null, null, null, null]
        };
    },
    ...
    componentDidMount() {
        ...
        const rawControlWidths = controls.map(c => c != null ? c.width : 0);
        const sumWidths = rawControWidths.reduce((sum, w) => sum + w, 0);
        const widthDiff = 318-sumWidths;
        if (widthDiff < 0) {
            return;
        }
        const numActiveControls = controls.reduce((count, c) => count + (c != null) ? 1 : 0);
        // Add 2 bits of padding between each control, hence 2*(controls-1)
        const padding = widthDiff/(2*(numActiveControls-1));
        const activeControlIndices = controls.map((c, i) => (c != null) ? i : null).filter(i => i != null);
        ...
        return (
            <ul className={Styles.control}>
                <If condition={item.canZoomTo}>
                    <li className={classNames(Styles.zoom, classList)} ref={c => this.zoomToRef = c}>
                ...
                <If condition={item.showsInfo}>
                    <li className={classNames(Styles.info, classList)} ref={c => this.aboutRef = c}>
                        <button type='button' onClick={this.previewItem} className={Styles.btn} title='info'>About This Data</button>
                    </li>
                </If>
                <If condition={canSplit}>
                    <li className={classNames(Styles.split, classList)} ref={c => this.splitRef = c}>
                ...
                <If condition={defined(item.linkedWcsUrl)}>
                    {/* WIP: Fix styles */}
                    <li className={classNames(Styles.info, classList)} ref={c => this.splitRef = c}>
                        <button type='button' onClick={this.exportData} className={Styles.btn} title='Export map data'>Export</button>
                    </li>
                </If>
                <li className={classNames(Styles.remove, classList)} ref={c => this.removeRef = c}>
